...
if not GEMINI_API_KEY:
    ...
    # transport="rest" evita gRPC, que consume mucha más memoria y en el
    # plan gratuito de Render (512 MB) puede causar que el proceso muera
    # con SIGKILL por quedarse sin RAM mientras espera la respuesta.
...
def build_diagnosis_prompt(language="es"):
    """Arma el prompt de diagnóstico. Si language='qu', le pide a Gemini que
    escriba los textos directamente en quechua chanka, para evitar una
    segunda llamada de traducción (más lenta y un punto extra de falla)."""
    if language == "qu":
        idioma_instr = (
            "Escribe TODOS los valores de texto libre (planta_identificada, "
            "plaga_o_problema, confianza, sintomas_observados, pasos, "
            "prevencion, urgencia) en quechua chanka (la variante de "
            "Apurímac y Cusco, Perú), con tono cálido y sencillo, como "
            "si le explicaras a un agricultor. IMPORTANTE: el campo "
            "\"severidad\" debe quedar EXACTAMENTE en español, con uno de "
            "estos tres valores literales: \"alta\", \"media\" o \"baja\" "
            "(nunca lo traduzcas, el sistema lo necesita así)."
        )
    else:
        idioma_instr = "Escribe todos los textos en español."

    return DIAGNOSIS_PROMPT + "\n\n" + idioma_instr


def call_gemini_with_retries(image_b64, media_type, language="es"):
    last_error = None
    image_bytes = base64.b64decode(image_b64)
    prompt = build_diagnosis_prompt(language)
...
def analyze():
    ...
    try:
        diagnosis = call_gemini_with_retries(image_b64, media_type, language=language)
    except Exception:
